...
また、Azure Cloud Shell、Azure PowerShellおよびTerraformを利用し、Azureリソースの管理およびInfrastructure as Code（IaC）による構築手法について学習しました。
...
graph TD
    ...
    B --> K[Azure Cloud Shell]
    B --> L[Azure PowerShell]
    B --> M[Terraform]
```
...
Terraformを利用し、Infrastructure as Code（IaC）によるAzureリソースの構築および管理の基本的な考え方について学習しました。
...
また、Windows ServerおよびUbuntu Serverの構築・リモート接続、Azure Cloud Shell、Azure PowerShellおよびTerraformを活用したAzureリソースの管理やIaCによる構築を通じて、Azure IaaS環境の基本的な設計・構築・管理に関する知識を習得しました。

今後は、Active Directory Domain Services（AD DS）、Azure Monitor、Azure Backup運用、Terraformによる環境構築の自動化に加え、可用性・セキュリティ・運用性を考慮したAzureインフラの設計についても継続的に学習を進めていきます。
